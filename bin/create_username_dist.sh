...
# Searches through the given directory for every failed_login_data.txt. Then we use awk to grab the name in column 4 and put it in holder.txt to be used later
for i in $Dir/*
do
    awk '{print $4}' $i/failed_login_data.txt >> holder.txt
done
...
./bin/wrap_contents.sh holder2.txt ./html_components/username_dist $Dir/username_dist.html
